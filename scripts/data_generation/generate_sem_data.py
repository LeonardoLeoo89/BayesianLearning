...
def generate_tsunami_sem(n_samples: int = 1000) -> pd.DataFrame:
    ...
    # Nonlinearity: Wave height is exponential with respect to earthquake magnitude
...
def generate_allergy_sem(n_samples: int = 1000) -> pd.DataFrame:
    ...
    # Nonlinearity: Birch pollen IgE has a quadratic relation with general pollen IgE
    B = 1.5 * P + 0.3 * (P**2) + np.random.normal(0, 0.5, n_samples)
    R = 1.1 * P + np.random.normal(0, 0.5, n_samples)
    
    # Nonlinearity: Asthma severity grows quadratically with Dust Mite IgE
...
def generate_train_delay_sem(n_samples: int = 1000) -> pd.DataFrame:
    ...
    # Nonlinearity: Passenger volume has a polynomial component for rush hour
    ...
    # Nonlinearity: Hub congestion scales quadratically with volume due to queues
    HC = 0.5 * (P**2) + np.random.normal(0, 0.5, n_samples)
    
    # Nonlinearity: Interaction effect between Infrastructure failure and Hub Congestion
